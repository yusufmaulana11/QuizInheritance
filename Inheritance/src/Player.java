...
//Pada class player saya memodifikasi dan menambahkan aksi pada player
//yaitu mencari item, jadi setelah melakukan aksi pada player
//kita bisa langsung mencari item di Ruangan, tanpa kembali ke GameEngine terlebih dahulu
//melainkan langsung dapat menjalankan aksi di Ruangan untuk mencari item.
...
public class Player {
    ...
    public void pilihanAksi() {
        ...
        //modifikasi
        //player ingin mencari item, player masuk ke ruangan
        urutPil++;
        subPil = 0;
        int pilihCariItem  = urutPil; //catat untuk pintu
        System.out.println("Cari Item");
        //hanya satu
        subPil++;
        System.out.printf("%d%d. Cari Item%n", urutPil, subPil);
        ...
        if (pil == pilInfoPlayer) {
            printPlayer();
        } else if (pil == pilihCariItem){
            System.out.println("Anda Masuk Ke Ruangan");
            System.out.println("==========");
            objGameInfo.getObjRuangan().pilihanAksi();
        }
        else {
